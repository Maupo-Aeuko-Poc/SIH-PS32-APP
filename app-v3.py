...
# Custom premium UI style sheet injection (Inter Font, CSS Card Elevation, Smooth Gradients)
st.markdown("""
<style>
    ...
    /* Modern minimalist custom cards */
    .landing-card {
        background: #ffffff;
...
# Hiding streamlit default header and footer for absolute design cleanliness
st.markdown("""
<style>
...
# ------------------------------------------------------------------------------
# 4.1 LANDING SCREEN (TWO MAJESTIC TILES)
# ------------------------------------------------------------------------------
if st.session_state.view == "landing":
    ...
    with logo_col2:
        st.markdown("<h1 style='text-align: center; font-size: 40px; font-weight: 800; letter-spacing: -1px; color: #0f172a; margin-bottom: 5px;'>SMART CROP PROCUREMENT PLATFORM</h1>", unsafe_allow_html=True)
        st.markdown("<p style='text-align: center; font-size: 16px; font-weight: 400; color: #64748b;'>An Enterprise Queue Scheduling, Dynamic FCFS Price Tuning & Inspector Audit Environment</p>", unsafe_allow_html=True)
        st.markdown("<p style='text-align: center; font-size: 11px; font-weight: 600; color: #94a3b8; letter-spacing: 2px;'>SMART INDIA HACKATHON • PROBLEM STATEMENT ID: SIH26032</p>", unsafe_allow_html=True)
        st.markdown("<div style='height: 1px; background-color: #e2e8f0; margin: 30px auto; width: 60%;'></div>", unsafe_allow_html=True)
    
    st.markdown("<br/>", unsafe_allow_html=True)
    
    # Two Elegant side-by-side tiles
    ...
    with tile_col1:
        ...
        <div class="landing-card">
            ...
            <h2 style="font-size: 24px; font-weight: 700; color: #0f172a; margin-bottom: 10px;">FARMER PORTAL</h2>
    ...
    with tile_col2:
        ...
        <div class="landing-card">
            <div style="font-size: 60px; margin-bottom: 15px;">🏢</div>
            <h2 style="font-size: 24px; font-weight: 700; color: #0f172a; margin-bottom: 10px;">ADMIN & INSPECTOR PORTAL</h2>
            <p style="font-size: 14px; color: #475569; line-height: 1.6; margin-bottom: 25px;">
    ...
    st.markdown("<div style='text-align: center; font-size: 12px; color: #94a3b8;'>Powered by Digital Public Infrastructure Frameworks • Government-Agent Linked ERP Core</div>", unsafe_allow_html=True)

# ------------------------------------------------------------------------------
# 4.2 CLEAN & CENTRED SIGN-IN PAGE
# ------------------------------------------------------------------------------
    ...
    with login_c2:
        ...
        if st.session_state.role == "farmer":
            st.markdown("<h2 style='text-align: center; font-size: 28px; font-weight: 700; margin-bottom: 10px;'>🌾 FARMER WORKSPACE ACCESS</h2>", unsafe_allow_html=True)
            st.markdown("<p style='text-align: center; color: #64748b; margin-bottom: 30px;'>Authenticate using your secure mobile credentials or register below</p>", unsafe_allow_html=True)
                ...
                if st.button("Verify Identity & Issue Account", use_container_width=True):
                    if new_name and new_phone and new_aadhaar:
                        ...
                        st.session_state.farmers_db[new_fid] = {
                            ...
                            "password": "pass",
                            "phone": new_phone,
                            "rating": 5.0,  # Seed newly registered farmer with perfect star rating
                            "trips": 0
                        }
        ...
        # Admin Portal Triple Credentials Verification Block
        else:
            st.markdown("<h2 style='text-align: center; font-size: 28px; font-weight: 700; margin-bottom: 10px;'>🏢 CONTROL CENTRE LOGIN</h2>", unsafe_allow_html=True)
            st.markdown("<p style='text-align: center; color: #64748b; margin-bottom: 30px;'>Triple-Factor administrative clearance required</p>", unsafe_allow_html=True)
...
# ------------------------------------------------------------------------------
# 4.3 SLEEK WORKSPACE INTERFACES (TABBED & SECTIONED LOGICAL FLOWS)
# ------------------------------------------------------------------------------
    ...
    if st.session_state.role == "farmer":
        ...
        with head_c1:
            st.markdown(f"<h1 style='font-size: 28px; font-weight: 800; color: #0f172a; margin: 0;'>🚜 FARMER PORTAL CONTROL CENTRE</h1>", unsafe_allow_html=True)
            farmer_profile = st.session_state.farmers_db[st.session_state.logged_in_farmer]
            st.markdown(f"<p style='color: #475569;'>Logged in as: <b>{farmer_profile['name']}</b> ({st.session_state.logged_in_farmer}) | Performance Rating: <b>{farmer_profile['rating']} ⭐</b></p>", unsafe_allow_html=True)
        with head_c2:
            st.markdown("<br/>", unsafe_allow_html=True)
        ...
        # Tabbed structure to arrange everything clean
            ...
            for order in st.session_state.buy_orders:
                ...
                <div style='background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 20px; margin-bottom: 20px;'>
                    <div style='display: flex; justify-content: space-between; align-items: center;'>
                        <h4 style='margin: 0; font-size: 18px; font-weight: 700; color: #0f172a;'>📦 Buy Request: {order['crop']} ({order['id']})</h4>
                        <span class="status-badge {'status-active' if remaining_cap > 0 else 'status-full'}">
                            {'Active Open' if remaining_cap > 0 else 'Queue Locked / Filled'}
                        </span>
                    </div>
                ...
                with info_col2:
                    if order["price_type"] == "Range":
                        ...
                        st.markdown(f"<span style='color: #10b981; font-weight: 600;'>⚡ Current Tiered Rate: ₹{estimated_tiered}/Quintal</span>", unsafe_allow_html=True)
                    else:
                ...
                # Interactive booking or Client-side view-only lock optimization
                if remaining_cap > 0:
                    with st.expander(f"📝 Book Drop-off Reservation for {order['id']}"):
                        ...
                        with book_col2:
                            # Dynamic pay and pricing estimation logic
                            price_secured = calculate_tiered_price(order, total_booked)
                            estimated_paycheck = int((book_qty / 100) * price_secured)
                            st.markdown(f"""
                            <div class='stat-container'>
                                <p style='margin: 0; font-size: 12px; color: #64748b; font-weight: 600;'>SECURED DYNAMIC RATE</p>
                                <h3 style='margin: 5px 0; font-size: 24px; color: #10b981;'>₹{price_secured} <span style='font-size: 11px; color: #64748b;'>/qtl</span></h3>
                                <p style='margin: 0; font-size: 11px; color: #475569;'>Estimated Payout: <b>₹{estimated_paycheck:,}</b></p>
                            </div>
                            """, unsafe_allow_html=True)
                    ...
                    # Render client-side read only data table
                ...
                st.markdown("<br/><hr style='border: 1px solid #f1f5f9;'/><br/>", unsafe_allow_html=True)
                ...
                for b in my_reservations:
                    ...
                    <div style='background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 20px; margin-bottom: 15px;'>
                        <div style='display: flex; justify-content: space-between;'>
                            <div>
                                <span style='font-size: 11px; font-weight: 700; color: #64748b;'>BOOKING REFERENCE: {b['id']}</span>
                                <h4 style='margin: 5px 0; font-size: 18px; font-weight: 700;'>🌾 {target_order['crop']} • Delivery</h4>
                            </div>
                            <div>
                                <span class="status-badge {'status-active' if b['status'] == 'Confirmed' else 'status-pending' if 'Completed' in b['status'] else 'status-full'}">
                                    {b['status']}
                                </span>
                            </div>
                        </div>
                    ...
                    st.markdown("<hr style='border: 1px solid #f1f5f9;'/><br/>", unsafe_allow_html=True)
        ...
        with farm_tab3:
            ...
            for log in st.session_state.push_notifications[:10]:
                st.markdown(f"""
                <div style='background-color: #f1f5f9; border-left: 4px solid #475569; padding: 12px 16px; border-radius: 4px; margin-bottom: 10px;'>
                    <span style='font-size: 13px; color: #1e293b; font-weight: 500;'>{log}</span>
                </div>
                """, unsafe_allow_html=True)
    ...
    # ------------------
        ...
        with head_a1:
            st.markdown("<h1 style='font-size: 28px; font-weight: 800; color: #0f172a; margin: 0;'>🏢 ADMIN & INSPECTOR CONTROL CENTRE</h1>", unsafe_allow_html=True)
            st.markdown("<p style='color: #475569;'>Authorized Administrative Session • Standard Security Key Active</p>", unsafe_allow_html=True)
        with head_a2:
            st.markdown("<br/>", unsafe_allow_html=True)
        ...
        with admin_tab1:
            ...
            st.write("Publish verified buy requests to the network. Use the integrated Localized Price Intelligence model to verify local market price suggests.")
            ...
            with ord_col2:
                ...
                <div style='background-color: #f8fafc; border: 1px solid #cbd5e1; border-radius: 8px; padding: 20px;'>
                    <h5 style='margin: 0 0 10px 0; font-size: 14px; font-weight: 700; color: #0f172a;'>📡 Localized Price Intelligence API</h5>
                ...
                if st.button("Run Region-Aware Market Price Lookup", use_container_width=True):
                    with st.spinner("Accessing dynamic local market averages..."):
                        ...
                        # Simulated smart geolocation pricing parameters
                        if "Delhi" in o_site:
        ...
        # 2nd Tab: Cargo Audit & Farmer Reputation Tuning (the Star rating system)
        with admin_tab2:
            ...
            if len(pending_evals) == 0:
                ...
                st.markdown(f"""
                <div style='background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 20px; margin-top: 15px; margin-bottom: 20px;'>
                    <h5 style='margin: 0 0 10px 0;'>👤 Farmer Profile Information</h5>
                    <p style='margin: 0;'>Farmer ID: <b>{target_b['farmer_id']}</b> | Name: <b>{target_b['farmer_name']}</b></p>
                    <p style='margin: 0;'>Declared Weight: <b>{target_b['qty']} kg</b> | Scheduled Delivery Slot: <b>{target_b['time']}</b></p>
                </div>
                """, unsafe_allow_html=True)
