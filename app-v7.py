...
# ==============================================================================
# 3. INTERFACE CONFIGURATION & HIGH-END ADAPTIVE THEME INJECTION
# ==============================================================================
st.set_page_config(
...
# Custom modern CSS stylesheet utilizing native theme variables for Light/Dark Mode
st.markdown("""
<style>
    ...
    /* Universal Role Selection Cards - Elegant Borders with dynamic accents */
    .landing-card-farmer {
    ...
    /* Modern Glassmorphic styled alert cards */
    .dashboard-panel {
        background: var(--secondary-background-color);
...
# ------------------------------------------------------------------------------
# 4.1 UNIVERSAL ROLE SELECTION LANDING SCREEN (v7 Spec - Plain, Minimalist & Accent Bordered)
# ------------------------------------------------------------------------------
if st.session_state.view == "landing":
    st.markdown("<br/><br/>", unsafe_allow_html=True)
    
    # Title Header Block
    st.markdown("<h1 style='text-align: center; font-size: 38px; font-weight: 800; letter-spacing: -1px; margin-bottom: 5px;'>SMART CROP PROCUREMENT PLATFORM</h1>", unsafe_allow_html=True)
    st.markdown("<p style='text-align: center; font-size: 15px; font-weight: 400; color: var(--text-color); opacity: 0.8;'>National Queue Scheduling, Real-Time Mandi Congestion Forecasting & Dynamic Capacity Recovery</p>", unsafe_allow_html=True)
    st.markdown("<p style='text-align: center; font-size: 10px; font-weight: 700; color: #10b981; letter-spacing: 2px;'>SMART INDIA HACKATHON • PSID: SIH26032</p>", unsafe_allow_html=True)
    st.markdown("<div style='height: 1px; background-color: var(--border-color); margin: 25px auto; width: 40%;'></div>", unsafe_allow_html=True)
    ...
    with tile_col1:
        ...
        <div class="landing-card-farmer">
            ...
            <h2 style="font-size: 22px; font-weight: 800; margin-bottom: 10px;">FARMER PORTAL</h2>
            <p style="font-size: 13.5px; opacity: 0.85; line-height: 1.6; margin-bottom: 25px;">
    ...
    with tile_col2:
        ...
        <div class="landing-card-admin">
            <div style="font-size: 55px; margin-bottom: 15px;">🏢</div>
            <h2 style="font-size: 22px; font-weight: 800; margin-bottom: 10px;">ADMIN & INSPECTOR</h2>
            <p style="font-size: 13.5px; opacity: 0.85; line-height: 1.6; margin-bottom: 25px;">
    ...
    st.markdown("<div style='text-align: center; font-size: 12px; opacity: 0.6;'>In Partnership with Department of Food & Public Distribution • Digital Public Infrastructure Framework</div>", unsafe_allow_html=True)

# ------------------------------------------------------------------------------
# 4.2 ADAPTIVE SIGN-IN SCREENS
# ------------------------------------------------------------------------------
    ...
    with login_c2:
        ...
        if st.session_state.role == "farmer":
            st.markdown("<h2 style='text-align: center; font-size: 26px; font-weight: 800; margin-bottom: 5px;'>FARMER PORTAL</h2>", unsafe_allow_html=True)
            st.markdown("<p style='text-align: center; opacity: 0.7; margin-bottom: 25px;'>Secure OTP-Backed Account Login</p>", unsafe_allow_html=True)
        ...
        # Admin Portal Authenticators
            ...
            st.markdown("<h2 style='text-align: center; font-size: 26px; font-weight: 800; margin-bottom: 5px;'>ADMIN CONTROL ACCESS</h2>", unsafe_allow_html=True)
            st.markdown("<p style='text-align: center; opacity: 0.7; margin-bottom: 25px;'>Secure Administrative Clearance Terminal</p>", unsafe_allow_html=True)
...
# ------------------------------------------------------------------------------
    ...
    if st.session_state.role == "farmer":
        ...
        with head_c1:
            st.markdown(f"<h1 style='font-size: 28px; font-weight: 800; margin: 0;'>🚜 FARMER CONTROL DASHBOARD</h1>", unsafe_allow_html=True)
            st.markdown(f"<p style='opacity: 0.8;'>Welcome back, <b>{farmer_profile['name']}</b> ({st.session_state.logged_in_farmer})</p>", unsafe_allow_html=True)
        with head_c2:
            st.markdown("<br/>", unsafe_allow_html=True)
            ...
            for order in st.session_state.buy_orders:
                ...
                <div class="dashboard-panel">
                    <div style='display: flex; justify-content: space-between; align-items: center;'>
                        <h4 style='margin: 0; font-size: 18px; font-weight: 700;'>🌾 {order['crop']} ({order['id']})</h4>
                ...
                with inf_c2:
                    st.markdown(f"💰 **Procurement Price:** **₹{order['msp_price']} / Quintal** (Strict Fixed MSP)")
                ...
                with inf_c3:
                    ...
                    st.markdown(f"""
                    <div class="stat-container" style="border-top: 4px solid {color};">
                        <p style="margin: 0; font-size: 11px; font-weight: 700; opacity: 0.7;">PREDICTIVE CONGESTION</p>
                        <h3 style="margin: 5px 0; font-size: 20px; color: {color};">{congestion}</h3>
                        <p style="margin: 0; font-size: 11px;">Est. Mandi Wait: <b>{25 if congestion == "MEDIUM" else 15} mins</b></p>
                    </div>
                    """, unsafe_allow_html=True)
                ...
                if remaining_cap > 0:
                    with st.expander(f"📝 Schedule Drop-off & Book Time Slot"):
                        ...
                        with book_col2:
                            ...
                            st.markdown(f"""
                            <div class="stat-container" style="margin-top: 10px;">
                                <p style="margin: 0; font-size: 11px; opacity: 0.7;">PAYCHECK ESTIMATION</p>
                                <h3 style="margin: 5px 0; color: #10b981;">₹{expected_paycheck:,}</h3>
                                <p style="margin: 0; font-size: 10px;">Calculated strictly at official MSP of ₹{order['msp_price']}/quintal</p>
                            </div>
                            """, unsafe_allow_html=True)
        ...
        with f_tab3:
            ...
            if len(my_bookings) == 0:
                ...
                for b in my_bookings:
                    ...
                    st.markdown(f"""
                    <div style='background-color: rgba(148, 163, 184, 0.05); border: 1.5px solid var(--border-color); border-radius: 8px; padding: 15px; margin-bottom: 12px;'>
                        <div style='display: flex; justify-content: space-between;'>
                            <div>
                                <span style='font-size: 11px; font-weight: 700; opacity: 0.7;'>TOKEN REF: {b['token_id']}</span>
                                <h4 style='margin: 5px 0;'>🌾 {target_order['crop']} • Delivery Schedule</h4>
                            </div>
    ...
    # ------------------
        ...
        with head_a1:
            st.markdown("<h1 style='font-size: 28px; font-weight: 800; margin: 0;'>🏢 ADMIN CONTROL CENTER</h1>", unsafe_allow_html=True)
        ...
        with adm_tab1:
            ...
            with oc2:
                ...
                <div class="stat-container">
                    <p style="margin: 0; font-size: 11px; font-weight: 700; opacity: 0.7;">PROCUREMENT INTELLIGENCE ENGINE</p>
        ...
        with adm_tab2:
            ...
            if len(pending_evals) == 0:
                ...
                st.markdown(f"""
                <div style='background-color: rgba(148, 163, 184, 0.05); border: 1px solid var(--border-color); border-radius: 8px; padding: 20px; margin-bottom: 20px;'>
                    <h5 style='margin: 0 0 10px 0; font-size: 14px;'>🔒 Internal Farmer Reliability Record (Security Vault)</h5>
                    <p style='margin: 0; font-size: 13px;'>Farmer Account ID: <b>{target_b['farmer_id']}</b> | Name: <b>{target_b['farmer_name']}</b></p>
                    <p style='margin: 0; font-size: 13px;'>Lifetime Bookings: <b>{farmer_record['bookings_made']}</b> | Successful Completions: <b>{farmer_record['completions']}</b></p>
                    <p style='margin: 0; font-size: 13px;'>No-Shows: <span style='color: #ef4444;'><b>{farmer_record['no_shows']}</b></span> | Late Cancellations: <b>{farmer_record['cancellations']}</b></p>
                </div>
                """, unsafe_allow_html=True)
        ...
        # TAB 3: System Analytics & Tech Architecture Specs (High-capacity design details)
        with adm_tab3:
            ...
            st.markdown("""
            <div style='background-color: rgba(148, 163, 184, 0.05); border: 1.5px solid var(--border-color); border-radius: 8px; padding: 20px; margin-top: 20px;'>
                <h5 style='margin: 0 0 10px 0;'>⚡ Real-Time High-Concurrency Scaling Blueprint</h5>
